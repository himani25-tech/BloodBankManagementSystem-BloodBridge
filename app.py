...
def signup():
    if request.method == 'POST':
        ...
        if role not in ('donor', 'hospital'):
            return jsonify({"success": False, "message": "Invalid role. Public sign-up is only available for Donor and Hospital accounts."})
...
def api_list_admins():
    admins = [
        ...
        for email, u in USERS.items() if u.get('role') == 'admin'
    ]
...
def api_create_admin():
    ...
    log_activity(session.get('name', 'Admin'), 'Created new admin', f"New admin: {name} ({email})")
    return jsonify({"success": True, "message": "New admin created successfully"})
